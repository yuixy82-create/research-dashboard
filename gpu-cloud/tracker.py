...
ANALYSIS = {
    ...
    "thesis": [
        "<b>AI 인프라 자금 = 데이터센터가 만들 현금흐름을 담보로 한 장기대출<br>: GPU도 담보에 들어가지만 감가가 빨라 핵심은 현금흐름임<br>★ 때문에 클라우드 가격이 매우 중요</b>",
        "현금흐름 = 클라우드 렌탈 가격 × 고객 신용도 (고객 등급에 따른 대출 금리 차이)",
        "대출 만기 5~6년 = GPU 수명과 비슷. 그런데 <b>계약으로 확보된 현금흐름은 3년뿐</b><br>: 남는 기간의 상환재원은 재계약가에 달림 = 클라우드 가격이 관건",
        "GPU 자산가치도 그 GPU가 벌어올 임대료로 매겨짐<br><b>→ 결국 모든 것이 클라우드 가격으로 수렴해, 클라우드 가격 하락 시 시스템 붕괴 위험</b>",
        "26.08 코어위브 DDTL 5.5가 그 첫 딜 — 재계약·잔존가치 리스크를 대주가 직접 떠안음<br>: 대주층도 사모 → 보험·연기금 → 공모채로 확산 중<br><b>★ 임대가에 돈이 걸린 사람이 넓어지는 중</b>",
...
""" + c_gap + """
<div class="note"><span class="key">빨강이 위 = 이자가 영업이익보다 큼(적자 구조). 좁혀지면 개선, 다시 벌어지면 경고</span><br>
